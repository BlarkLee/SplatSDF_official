...
from project.nerf.trainers.base import BaseTrainer
from project.splatsdf.utils.misc import get_scheduler, eikonal_loss, curvature_loss
from project.gaussians.gaussian_renderer import render
...
class Trainer(BaseTrainer):
    ...
    def _compute_loss(self, data, mode=None):
        if mode == "train":
            ...
            # Only for objects case when sampling rays from valid mask
            data['image_sampled'] = data['image_sampled'].squeeze(0).permute(1,2,0)
            data['image_sampled'] = data['image_sampled'].view(-1, data['image_sampled'].shape[-1])
            data["image_sampled"] = data["image_sampled"][data['ray_idx']]
