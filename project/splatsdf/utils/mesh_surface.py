...
@torch.no_grad()    
def extract_mesh_surface(sdf_func, sdf_func_enc, bounds, intv, block_res=64, texture_func=None, filter_lcc=False):
    ...
    for it, data in enumerate(data_loader):
        xyz = data["xyz"][0]
        xyz_cuda = xyz.cuda()
        xyz_cuda_shape = xyz_cuda.shape
        
        gaussian_agg_feats = None
        sdf_cuda = sdf_func(xyz_cuda, gaussian_agg_feats)[..., 0]
        sdf_cuda = sdf_cuda.view(xyz_cuda.shape[0], xyz_cuda.shape[1], xyz_cuda.shape[2])
        sdf = sdf_cuda.cpu()
        mesh = marching_cubes(sdf.numpy(), xyz.numpy(), intv, texture_func, filter_lcc, gaussian_agg_feats)
...
@torch.no_grad()
def extract_texture(xyz, neural_rgb, neural_sdf, appear_embed, gaussian_feat):
    num_samples, _ = xyz.shape
    xyz_cuda = torch.from_numpy(xyz).float().cuda()[None, None]  # [N,3] -> [1,1,N,3]
    sdfs, feats = neural_sdf(xyz_cuda, gaussian_feat)
...
def marching_cubes(sdf, xyz, intv, texture_func, filter_lcc, gaussian_feat):
    # marching cubes
    #print("sdf", sdf.shape, np.unique(sdf))
    V, F = mcubes.marching_cubes(sdf, 0.)
    #print("V, F", V.shape, F.shape)
    if V.shape[0] > 0:
        V = V * intv + xyz[0, 0, 0]
        if texture_func is not None:
            C = texture_func(V, gaussian_feat=gaussian_feat)
