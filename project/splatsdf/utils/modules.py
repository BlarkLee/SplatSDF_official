...
from project.splatsdf.utils.spherical_harmonics import get_spherical_harmonics
from project.splatsdf.utils.mlp import MLPforNeuralSDF
from project.splatsdf.utils.misc import get_activation
from project.nerf.utils import nerf_util
...
class NeuralSDF(torch.nn.Module):
    ...
    def build_mlp(self, cfg_mlp, input_dim=3):
        ...
        layer_dims = [input_dim] + [cfg_mlp.hidden_dim] * cfg_mlp.num_layers + [cfg_mlp.hidden_dim] #when replace
        #layer_dims = [input_dim+64] + [cfg_mlp.hidden_dim] * cfg_mlp.num_layers + [cfg_mlp.hidden_dim] #when concat
    ...
    def forward(self, points_3D, gaussian_feat=None, with_sdf=True, with_feat=True, mode='concat', ray_idx=None, surface_idx=None, surface_points=None, surface_fusion=True):
        points_enc = self.encode(points_3D)  # [...,3+LD]
        if gaussian_feat is not None:
            if mode == 'concat':
                points_enc = torch.cat([points_enc, gaussian_feat], dim=-1)
            elif mode == 'replace':
                gaussian_feat = torch.cat([surface_points, gaussian_feat], dim=-1)
                if surface_fusion:
                    points_enc[:,ray_idx,surface_idx,:] = gaussian_feat[:,ray_idx,0,:] # for only one surface points per ray 
                else: # for surface points more than 1 per ray
                    surface_idx_sub = surface_idx[ray_idx, :]  # Shape: [98, 5]
                    ray_indices = ray_idx.unsqueeze(1).expand(-1, 5)  # Shape: [98, 5]
                    points_enc[0, ray_indices, surface_idx_sub, :] = gaussian_feat[0, ray_idx, :, :]  # Shape: [98, 5, 131]
                

                '''
                #concat instead of replace
                combined_features = torch.cat((points_enc[:,ray_idx,surface_idx,:], gaussian_feat[:,ray_idx,0,:]), dim=-1)
                #print("combined_features", combined_features.shape)
                projection_layer = nn.Linear(int(gaussian_feat.shape[-1]*2), int(gaussian_feat.shape[-1])).cuda()
                batch_size, num_rays, feature_dim = combined_features.shape
                combined_features_reshaped = combined_features.view(-1, feature_dim)  # shape: [1*512*128, 262]
                projected_features_reshaped = projection_layer(combined_features_reshaped)  # shape: [1*512*128, 131]
                projected_features_reshaped = F.relu(projected_features_reshaped)
                points_enc[:,ray_idx,surface_idx,:] = projected_features_reshaped.view(batch_size, num_rays, -1)  # shape: [1, 512, 128, 131]'''
                
        sdf, feat = self.mlp(points_enc, with_sdf=with_sdf, with_feat=with_feat)
        return sdf, feat  # [...,1],[...,K]

    def sdf(self, points_3D, gaussian_feat=None):
        return self.forward(points_3D, gaussian_feat, with_sdf=True, with_feat=False)[0]

    def encode(self, points_3D, cat_pts=True):
    ...
    def compute_gradients(self, x, gaussian_feat=None, training=False, sdf=None):
        ...
        if self.cfg_sdf.gradient.mode == "analytical":
            ...
            if self.cfg_sdf.gradient.taps == 6:
                ...
                sdf1 = self.sdf(x + k1 * eps, gaussian_feat)  # [...,1]
                sdf2 = self.sdf(x + k2 * eps, gaussian_feat)  # [...,1]
                sdf3 = self.sdf(x + k3 * eps, gaussian_feat)  # [...,1]
                sdf4 = self.sdf(x + k4 * eps, gaussian_feat)  # [...,1]
...
class BackgroundNeRF(torch.nn.Module):
    ...
    def forward(self, points_3D, rays_unit, app_outside, gaussian_feat=None):
