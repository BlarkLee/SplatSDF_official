...
__global__ void query_neigh_along_ray_layered(
    const float* in_data,   // B * N * 3
    const int B,
    const int D,               // all points along each ray  // num. samples along each ray e.g., 128
    ...
    const float *sample_loc,       // B * R * D * 3 // B * R * SR * 3
    const int *sample_loc_mask,       // B * R * D // B * R * SR
    int *sample_pidx,       // B * R * D * K // B * R * SR * K
    const int NN
) {
    int index =  blockIdx.x * blockDim.x + threadIdx.x; // index of gpu thread
                ...
                for (int z =  max(-frustz, -layer); z < min(d_grid_size[2] - frustz, layer + 1); z++) {
                    ...
                    if (occ_indx >= 0) {
                        for (int g = 0; g < min(P, occ_numpnts[occ_indx]); g++) {
                            ...
                            //std::cout << "xyz2" << xyz2 << std::endl;
                            //std::cout << "radius_limit2" << radius_limit2 << std::endl;
                            //printf("xyz2: %f\n", xyz2);
                            //printf("radius_limit2: %f\n", radius_limit2);
...
std::vector<torch::Tensor> woord_query_grid_point_index_cuda(
  ...
  //auto sample_loc_tensor = torch::zeros({B, R, D, 3}, torch::dtype(torch::kFloat32).device(torch::kCUDA));
  auto sample_pidx_tensor = torch::full({B, R, D, K}, -1, torch::dtype(torch::kInt32).device(torch::kCUDA));
  ...
  if(R > 0){
        ...
        sample_pidx_tensor = torch::masked_select(sample_pidx_tensor, masked_valid_ray.unsqueeze(-1).unsqueeze(-1).expand({-1, -1, D, K})).reshape({B, R, D, K});
        //sample_loc_tensor = torch::masked_select(sample_loc_tensor, masked_valid_ray.unsqueeze(-1).unsqueeze(-1).expand({-1, -1, D, 3})).reshape({B, R, D, 3});
  }

  //return {sample_pidx_tensor, sample_loc_tensor, ray_mask_tensor.toType(torch::kInt8)};
  return {sample_pidx_tensor, ray_mask_tensor.toType(torch::kInt8)};
}