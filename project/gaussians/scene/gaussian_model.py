...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        self.max_sh_degree = sh_degree #0 #sh_degree  
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        fused_sh = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().cuda())
        
        features = torch.zeros((fused_rgb.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()
        features[:, :3, 0 ] = fused_sh #color
        ...
        self._features_rest = nn.Parameter(fused_rgb[:,None,:].contiguous().requires_grad_(True))
    ...
    def load_ply(self, path, scale_mat=None):
    ...
    def _prune_optimizer(self, mask, optimizer):
        optimizable_tensors = {}
        gaussian_param_names = ["xyz","f_dc","f_rest","opacity","scaling","rotation"]
        for group in optimizer.param_groups:
            if group["name"] not in gaussian_param_names:
                continue
    ...
    def get_normals(self, estimate_from_points=False, neighborhood_size:int=32):
        ...
        if estimate_from_points:
            ...
            binded_to_surface_mesh = False
            if binded_to_surface_mesh:
