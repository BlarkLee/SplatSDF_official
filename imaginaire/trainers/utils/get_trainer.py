...
def get_optimizer(cfg_optim, model):
    ...
    l_gaussian = [
            {'params': [model.gaussians._xyz], 'lr': 0.0075, "name": "xyz"},
            {'params': [model.gaussians._features_dc], 'lr': 0.0025, "name": "f_dc"},
            {'params': [model.gaussians._features_rest], 'lr': 0.00125, "name": "f_rest"},
            {'params': [model.gaussians._opacity], 'lr': 0.05, "name": "opacity"},
            {'params': [model.gaussians._scaling], 'lr': 0.005, "name": "scaling"},
            {'params': [model.gaussians._rotation], 'lr': 0.001, "name": "rotation"}
        ]
    ...
    for name, param in model.named_parameters():
        l.append({'params': param, "lr": optimizer_kwargs['lr'], "weight_decay": optimizer_kwargs['weight_decay'], "name": name})    

    optim = optimizer_class(l, **optimizer_kwargs)
