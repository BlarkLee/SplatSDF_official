...
class BaseTrainer(object):
    ...
    def end_of_iteration(self, data, current_epoch, current_iteration):
        ...
        if current_iteration % self.cfg.checkpoint.save_iter == 0 or \
            ...
            self.checkpointer.save(current_epoch, current_iteration, self.gaussian)
    ...
    def train_step(self, data, epoch, last_iter_in_epoch=False):
        r"""One training step.
        ...
        for current_epoch in range(start_epoch, cfg.max_epoch):
            ...
            for it, data in enumerate(data_loader_wrapper):
                with profiler.profile(enabled=profile,
                    ...
                    self.train_step(data, current_epoch, last_iter_in_epoch=(it == len(data_loader) - 1))
    ...
    def test(self, cfg, data_loader, single_gpu=False, profile=False, show_pbar=False, output_dir=None, inference_args=None):
        ...
        for current_epoch in range(start_epoch, cfg.max_epoch):
            ...
            self.sdfs_zero_loss = 0
            self.sdfs_gradients_loss = 0
            for it, data in enumerate(data_loader_wrapper):
...
class Checkpointer(object):
    ...
    def save(self, current_epoch, current_iteration, gaussian, latest=False):
        ...
        gaussian_checkpoint_file = f'epoch_{current_epoch:05}_iteration_{current_iteration:09}_pointcloud.ply'
        gaussian_checkpoint_path = self._get_full_path(gaussian_checkpoint_file)
